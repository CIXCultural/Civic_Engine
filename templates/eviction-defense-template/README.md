...
- Community paralegals and tenant advocates who want a digital tool without hiring developers
- Researchers and policy organizations publishing tenant rights information
- Any jurisdiction, any language

## What you will build

A decision-tree navigator that guides tenants through eviction defense procedures in plain language, ending with a personalized action checklist and direct links to free legal resources. It runs entirely in the browser — no data leaves the device.
...
## Step 5 — Remove template metadata
...
Common errors:
- Missing required fields (`version`, `domain`, `jurisdiction`, `entryNode`, `nodes`)
- A `next` reference pointing to a node that doesn't exist
- A `terminal` node with no `items`
